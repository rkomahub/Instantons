...
double S_full(const std::vector<double> &x, double a, double eta) {
  ...
  for (int i = 0; i < N; ++i) {
    const int ip = (i + 1) % N;
    const double dx = x[ip] - x[i];
    const double Vf = std::pow(x[i] * x[i] - eta * eta, 2);
    S += dx * dx / (4.0 * a) + a * Vf;
  }
...
void metropolis_alpha(std::vector<double> &x, double a, double eta,
  ...
  for (int sweep = 0; sweep < sweeps; ++sweep) {
    for (int i = 0; i < N; ++i) {
      ...
      const double dS_kin =
          (std::pow(x_new - x[im], 2) + std::pow(x[ip] - x_new, 2) -
           std::pow(x_old - x[im], 2) - std::pow(x[ip] - x_old, 2)) /
          (4.0 * a);
    ...
    for (int i = 0; i < N; ++i) {
      ...
      if (u01(gen) < std::exp(-dS)) {
        ...
        if (enforce_orientation && i0 >= 0) {
          const int im0 = (i0 - 1 + N) % N;
          const int ip0 = (i0 + 1) % N;
...
void run_qmidens_analysis(std::mt19937 &gen) {
  ...
  for (double beta : beta_list) {
    ...
    auto stderr_of = [](const std::vector<double> &v, double mean) {
      if (v.size() < 2)
        return 0.0;
    ...
    const double F_mean = mean_of(F_vals);
    const double F_err = stderr_of(F_vals, F_mean);

    const double I_fine_mean = mean_of(I_fine_vals);
    const double I_coarse_mean = mean_of(I_coarse_vals);
    const double delta_mean = mean_of(delta_vals);
