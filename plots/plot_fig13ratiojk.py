...
# =============================
# Exact correlator (ground-state dominated)
# =============================
...
def jackknife_meff_ratio(C_trials, tau):
    ...
    sumC = C_trials.sum(axis=0)
    C_loo = (sumC[None, :] - C_trials) / (T - 1)

    m_loo = np.array([meff_ratio_from_C(C_loo[k], dt) for k in range(T)])
...
# =============================
# Load HEATED-RILM data (Fig.13)
# =============================
df = pd.read_csv(DATA / "fig13_heated_rilm_correlators.csv")
tau = df["tau"].values
...
dfT = pd.read_csv(DATA / "fig13_trials_correlators.csv")

taus = np.sort(dfT["tau"].unique())
if len(taus) != len(tau) or np.max(np.abs(taus - tau)) > 1e-12:
    raise RuntimeError("tau grid in fig13_trials_correlators.csv does not match fig13_heated_rilm_correlators.csv")
...
C3_exact = exact_correlator_ground_state(E, V, X3, tau, nstates=40)

# =============================
# Jackknife effective masses (ratio)
# =============================
m1_mc, dm1_mc = jackknife_meff_ratio(C1_trials, tau)
m2_mc, dm2_mc = jackknife_meff_ratio(C2conn_trials, tau)
...
print("\n=== Exact and estimate <x^2> ===")
print(f"<x^2>_exact = {x2_exact:.6f}")
print(f"<x^2>_heated-RILM (from plateau) ≈ {x2_mc_est:.6f}")
...
ax1.errorbar(tau, C1_mc, yerr=dC1, fmt='o', ms=3, label=r"Heated RILM $\langle x x\rangle$")
ax1.errorbar(tau, C2raw_mc, yerr=dC2raw, fmt='o', ms=3, label=r"Heated RILM $\langle x^2 x^2\rangle$")
ax1.errorbar(tau, C3_mc, yerr=dC3, fmt='o', ms=3, label=r"Heated RILM $\langle x^3 x^3\rangle$")
...
ax2.errorbar(tau, m1_mc, yerr=dm1_mc, fmt='o', ms=3, label=r"Heated RILM $x$")
ax2.errorbar(tau, m2_mc, yerr=dm2_mc, fmt='o', ms=3, label=r"Heated RILM $x^2$ (conn)")
ax2.errorbar(tau, m3_mc, yerr=dm3_mc, fmt='o', ms=3, label=r"Heated RILM $x^3$")
...
plt.tight_layout()
plt.savefig(FIGS / "Figure_13ratiojk.png", dpi=250)
plt.show()